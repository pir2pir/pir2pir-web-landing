...
import {renderSitemap} from './src/sitemap';
    ...
    configureServer(server) {
      ...
      return () => {
        server.middlewares.use(async (req, res, next) => {
          ...
          // Generated by the build rather than kept in public/, so the dev server has to render them
          // too — otherwise the one link in the head that cannot be checked by reading the page is
          // also the one that 404s here and nowhere else.
          ...
          if (url === '/sitemap.xml') {
            // No lastmod: the build reads it from git, and there is nothing to date until then.
            return send(res, 'application/xml', renderSitemap());
          }
