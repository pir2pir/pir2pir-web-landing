...
type LogoProps = {
  ...
  decorative?: boolean;
  /**
   * Draw the mark in `currentColor` instead of the brand gradient. For the one place the mark sits on
   * the brand gradient itself, where rose on rose is a mark you cannot see — the card keeps its
   * gradient and the mark takes the card's text colour, which is the pair that has contrast.
   */
  solid?: boolean;
};

/**
 * The Pir2Pir mark. An inline SVG's defs are global to the document, so the gradient id is unique
 ...
 */
export function Logo({height = 59, className, decorative = false, solid = false}: LogoProps) {
  ...
  return (
    ...
    >
      ...
      {/* No gradient to define when the mark is drawn flat, and no unused id left in the document. */}
      {solid ? null : (
        <defs>
          ...
          <linearGradient id={gradientId} gradientUnits="userSpaceOnUse" x1="0" y1="8" x2="108.11" y2="51">
            <stop stopColor="#E11D48" />
            <stop offset="1" stopColor="#EA580C" />
          </linearGradient>
        </defs>
      )}
      <g fill={solid ? 'currentColor' : `url(#${gradientId})`}>
